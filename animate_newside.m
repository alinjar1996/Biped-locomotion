...
function [] = animate_newside(tskip)
disp('------------------------------------------------------------------');
...
%draw plane finish
    ...
    plot3(L1Z(i,:),-L1X(i,:),L1Y(i,:),'r',L2Z(i,:),-L2X(i,:),L2Y(i,:),'b','linewidth',3);
    ...
    fill3(BZ(i,:),-BX(i,:),BY(i,:),[1 1 ],'linewidth',2.5)
    hold on
    fill3(LZ(i,:),-LX(i,:),LY(i,:),[1 1 ],'linewidth',2.5)

    axis([zmin zmax -xmax -xmin ymin ymax]);
    fff = 28;
    set(gca,'fontsize',fff,'fontweight','n','fontname','euclid','linewidth',1.5, 'fontangle', 'normal')
    xlabel('$z$ (m)','fontweight','n','fontsize',fff, 'interpreter','latex');
    ylabel('$x$ (m)','fontweight','n','fontsize',fff, 'interpreter','latex');
    zlabel('$y$ (m)','fontweight','n','fontsize',fff, 'interpreter','latex');
    %title(t,'fontweight','n','fontsize',14)
     view(-90,0) %Side view
    % view(0,90) %Front view
    hold off % Comment it if you want stick figure
    grid on;
    drawnow;
%     pause(0.05)
end
